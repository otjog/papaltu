    ...
    <!-- Не показываем стрелки движения влево, если мы на первой странице -->
        <ul class="page_nav d-flex flex-row">
            @if($products->currentPage() != 1)
                <li><a href="{{$products->appends(old())->url(1)}}"><i class="fa fa-angle-double-left"></i></a></li>
                <li><a href="{{$products->appends(old())->previousPageUrl()}}"><i class="fa fa-angle-left"></i></a></li>
            @endif
        ...
        <!-- Выводим в цикле кнопки, текущая страница не имеет ссылки на саму себя -->
            @for($i = $startPage; $i <= $endPage && $i <= $products->lastPage(); $i++)
                @if($products->currentPage() != $i)
                    <li><a href="{{$products->appends(old())->url($i)}}">{{$i}}</a></li>
                @else
                    <li><span>{{$i}}</span></li>
                @endif
            @endfor
        <!-- Не показываем стрелки движения вправо, если мы на последеней странице -->
            @if($products->currentPage() != $products->lastPage())
                <li><a href="{{$products->appends(old())->nextPageUrl()}}"><i class="fa fa-angle-right"></i></a></li>
                <li><a href="{{$products->appends(old())->url($products->lastPage())}}"><i class="fa fa-angle-double-right"></i></a></li>
            @endif
        </ul>
    @endif
</div>